...
export const SETTINGS_SCHEMA = [
  ...
  },
  {
    ...
    items: [
      ...
      { key: 'curseOnNoSpace', label: 'Curse drawn with no empty cell', type: 'select', def: 'discard',
      ...
      { key: 'peekCount', label: 'Upcoming cards shown', type: 'range', min: 0, max: 3, step: 1, def: 1 },
  ...
  },
  {
    group: 'Walls & timing',
    items: [
      { key: 'clock', label: 'Clock', type: 'select', def: 'time',
      ...
      { key: 'goalTurns', label: 'Goal time limit (turns)', type: 'range', min: 2, max: 40, step: 1, def: 10 },
  ...
  },
  {
    ...
    items: [
      ...
      { key: 'wardCostRetreat', label: 'Ward cost: push a wall back one step (0 = not allowed)', type: 'range', min: 0, max: 3, step: 1, def: 1 },
      { key: 'rerollTimer', label: 'A replaced goal', type: 'select', def: 'keep',
        options: [['keep', 'Keeps the remaining time'], ['reset', 'Starts with a fresh timer']] },
    ],
  },
  {
